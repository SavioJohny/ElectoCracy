...
try {
    ...
    // Verify the class election exists
    $stmt = $pdo->prepare("
    ...
    if ($action === 'toggle_results_publication') {
        ...
        $class_name = $election['class_name'] ?? 'Unknown Class';
        $response['message'] = "Class election results for {$class_name} have been {$action_text} successfully.";
        
        // Log the action
        error_log("Commissioner {$current_user['fname']} {$current_user['lname']} {$action_text} class election results for {$class_name} (election {$election_id})");
